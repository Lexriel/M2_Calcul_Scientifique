...
void loadInstances(char* filename, char* u)
{
  ifstream data_file;
  int i=0;
...
int main(int argc, char* argv[])
{
  ...
  int cout_ext_indel   = -2;   // extension d'indel
  int cout_subs  = -4;         // substitution
  int cout_match = +5;         // correspondance
  int bool_indel = 0;

  // Chargement des mots étudiés et leurs longueurs
  taille1 = comptage(argv[1]); //strlen(argv[1]);
  taille2 = comptage(argv[2]);
  mot1 = (char*) malloc( taille1 * sizeof(char) );
  mot2 = (char*) malloc( taille2 * sizeof(char) );
  loadInstances(argv[1], mot1);
  loadInstances(argv[2], mot2);
  ...
  score[0][0] = 0;
  for (i=1; i<I; i++)
    score[i][0] = cout_debut_indel + (i-1) * cout_ext_indel;
  for (j=1; j<J; j++)
    score[0][j] = cout_debut_indel + (j-1) * cout_ext_indel;
      ...
      for (i=1; i<I; i++)
        {
          ...
          if (bool_indel == 0) // début d'indel
            {
              ins = score[i][j-1] + cout_debut_indel;   // insertion
              del = score[i-1][j] + cout_debut_indel;   // délétion
              bool_indel = 1;
            }
          else                 // extension d'indel
            {
              ins = score[i][j-1] + cout_ext_indel;     // insertion
              del = score[i-1][j] + cout_ext_indel;     // délétion
            }
 
          score[i][j] = maximum(match_subs, ins, del);  // calcul du score en (i,j)

          if (score[i][j] == match_subs)
            bool_indel = 0;
        }
    }
  ...
  while ( (i > 0) || (j > 0) )
    {
      ...
      if ( (i > 0) && (j > 0) )
        {
          if ( score[i-1][j-1] + cout_match == score[i][j] )
            {
          ...
          if ( score[i-1][j-1] + cout_subs == score[i][j] )
            {
      ...
      if (i > 0)
	{
	  ...
	  if ( score[i-1][j] + cout_ext_indel == score[i][j] )
	    {
	      chemin[compteur++] = INS;
	      i--;
	      continue;
      ...
      if (j > 0)
	{
	  ...
	  if ( score[i][j-1] + cout_ext_indel == score[i][j] )
	    {
	      chemin[compteur++] = DEL;
	      j--;
	      continue;
